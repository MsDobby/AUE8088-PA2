...
def ap_per_class(tp, conf, pred_cls, target_cls, num_imgs, plot=False, save_dir=".", names=(), eps=1e-16, prefix=""):
    """
    ...
    ap, p, r, fppi, mr = np.zeros((nc, tp.shape[1])), np.zeros((nc, 1000)), np.zeros((nc, 1000)), np.zeros((nc, 1000)), np.zeros((nc, 1000))
    ...
    for ci, c in enumerate(unique_classes):
        ...
        # TODO : FPPI (False Positive Per Image)
        # fppi_ = fpc[:, 0] / num_imgs  # FP per an images
        # fppi_ = fpc / num_imgs  # FP per an images
        # fppi[ci] = np.interp(-px, -conf[i], fppi_[:, 0], left=0)
        fppi[ci] = np.interp(-px, -conf[i], fpc[:, 0] / num_imgs, left=0)
        
        # TODO : miss rate
        miss_rate = 1 - recall 
        mr[ci] = np.interp(-px, -conf[i], miss_rate[:, 0], left=0)
    ...
    p, r, fppi, mr, f1 = p[:, i], r[:, i], fppi[:, i], mr[:, i], f1[:, i]
    tp = (r * nt).round()  # true positives
    fp = (tp / (p + eps) - tp).round()  # false positives
    return tp, fp, p, r, fppi, mr.mean(), f1, ap, unique_classes.astype(int)
